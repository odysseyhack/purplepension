...
const config = require('../data/strava_config.js');
...
router.get("/getacces", function (req, res ) {
    ...
    var redirectUri = 'http://localhost:3000/strava/receiveacces';
...
router.get("/receiveacces", function (req, res) {
    ...
    request.post(uri, args, function(err, response, body) {
        if(!err) {
            ...
            return res.redirect('/strava/activities?access_token=' + access_token);
            //res.send(body)
        }
        else {
            console.log(err);
...
router.get('/activities', function (req, res) {
    ...
    request.get(uri, args, function(err, response, body) {
        if(!err) {
            if(body.message === 'Authorization Error') return res.redirect('/strava/getacces');
            res.send(body)
        }
        else {
            console.log(err);
